 ...
 * - analyzePrompt(): backwards-compat wrapper so content.ts doesn't need changes
 ...
 *
 * each entry has:
 * - pattern: the regex to match against the student's text
 ...
 * "how do I implement X" is executive even though it doesn't say "give me code".
 */
const EXECUTIVE_PATTERNS: PatternConfig[] = [
  {
    ...
    // \b = word boundary so "rewrite" doesn't trigger this
    pattern:
  ...
  {
    // "what is the code", "what's the solution", "what is the answer"
 ...
 * we don't block these, just recognize them (and could affirm the student in the future).
 *
 * NOTE: adaptive patterns don't have a suggestion field — there's nothing to redirect.
 */
 ...
 * =====================
 * the default classifier. runs the student's text against the pattern lists above.
 *
 * to use a different classifier instead, extend PromptClassifier and swap it in
 * via getActiveClassifier() in content.ts. you don't need to touch this class.
 *
 ...
 * kind of executive). that's a known limitation — an LLM classifier would do better.
 */
export class RegexPromptClassifier extends PromptClassifier {
  classify(submission: string): AnalysisResult {
    // start with a clean empty result — we'll fill it in below
 ...
 *
 * right now this falls back to regex — it's just a stub so you can see
 ...
 * 2. your backend hits Gemini with the student's prompt + a classification prompt
 ...
 * to go async (needed for API calls), you'd need to change the call site too.
 ...
 * to switch classifiers, just change what's returned here.
 *
 ...
 * analyzePrompt()
 * ===============
 * thin wrapper around the active classifier's classify() method.
 * kept so content.ts doesn't need changes — it still calls analyzePrompt() directly.
 *
 ...
 * ======================
 * checks the AI's response (not the student's prompt) for signs of code.
 * used to decide whether to blur the response.
 *
 * this is intentionally kept as a plain function (not a classifier subclass)
 * because it operates on a different input — the assistant's output, not the student's.
 *
